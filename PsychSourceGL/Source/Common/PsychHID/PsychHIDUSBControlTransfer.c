...
PsychError PSYCHHIDUSBControlTransfer(void) 
{
	...
	if (bmRequestType == 0xC0) {
		if (wLength <= 0) {
			PsychErrMsgTxt("(PSYCHHIDGenericUSBControlTransfer) wLength must be > 0 for an out command");
		}
		...
		PsychErrMsgTxt("(PSYCHHIDGenericUSBControlTransfer) bmRequestType must be 0x40 or 0xC0");
	}
	
	// Make the actual control request.
	if (PsychHIDControlTransfer(usbHandle, (psych_uint8)bmRequestType, (psych_uint16)wValue, (psych_uint16)wIndex, (psych_uint16)wLength, (void*)buffer) == false) {
		PsychErrMsgTxt("(PSYCHHIDGenericUSBControlTransfer) USB Control transfer failed.");
	}
