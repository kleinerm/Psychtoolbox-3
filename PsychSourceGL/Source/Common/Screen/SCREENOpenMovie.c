...
static char synopsisString[] = 
		...
		"PsychPortAudio sound output, otherwise movie playback may hang. 4 = Use a special YUV raw decoding format "
		"instead of RGBA for the video frames. This format is especially efficient when used with popular codecs like "
		"H264, but it requires use of a special YUV shader for drawing the texture with the video image. The shader "
		"is automatically attached to the texture, but only used during Screen('DrawTexture'). If you try to use the "
		"video texture for anything but 'DrawTexture', you may experience unexpected results. Use of flag 4 is supported "
		"on any system that supports GLSL shaders and may be even more efficient than use of flag 1, with the mentioned "
		"caveats. A setting of 8 or 16 will decode the video at a lower resolution (8 == half resolution, 16 == quarter "
		"resolution). This sacrifices video quality but can increase the decoding speed for speed critical movie "
		"playback. Low resolution playback may not be supported by all codecs.\n"
