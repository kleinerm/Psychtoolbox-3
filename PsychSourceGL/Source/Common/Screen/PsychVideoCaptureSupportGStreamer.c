								   ...
								   int reqdepth, int num_dmabuffers, int allow_lowperf_fallback, char* targetmoviefilename, unsigned int recordingflags)
{
	...
	*capturehandle = -1;
	...
	if (firsttime) {
		...
		if(!gst_init_check(NULL, NULL, &error)) {
			if (printErrors && error) {
				...
				g_error_free(error);
				PsychErrorExitMsg(PsychError_system, "GStreamer initialization failed! Video capture engine out of order.");
			} else
				return(FALSE);
		}
...
#if PSYCH_SYSTEM == PSYCH_OSX
    // Specific deviceInd	ex requested, instead of auto-select?
    if (deviceIndex > 0) {
    ...
    case 2:
	    ...
	    if (PsychPrefStateGet_Verbosity()>1) printf("PTB-WARNING: Video capture engine doesn't support requested Luminance+Alpha format. Will revert to pure Luminance inste	ad...\n");
    case 1:
	    reqdepth = 1;
    ...
    if (PsychPrefStateGet_Verbosity()>4) printf("PTB-INFO: Final configuration string passed to GStreamer library is:\n%s\n", config);
