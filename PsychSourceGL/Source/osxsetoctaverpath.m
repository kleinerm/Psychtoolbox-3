...
% files should always find a dylib for the currently running Octave.
    ...
    libdir = '/usr/local/opt/octave/lib/octave/4.4.0';

    % Replace absolute path to liboctinterp.5.dylib with @rpath:
    system(['install_name_tool -change ' libdir '/liboctinterp.5.dylib @rpath/liboctinterp.5.dylib ' mexfname]);

    % Replace absolute path to liboctave.5.dylib with @rpath:
    system(['install_name_tool -change ' libdir '/liboctave.5.dylib @rpath/liboctave.5.dylib ' mexfname]);

    % Add one single rpath: @loader_path. This is the path to our folder where our
    % mex file is stored. If we place copies of liboctave.5.dylib and liboctinterp.5.dylib
