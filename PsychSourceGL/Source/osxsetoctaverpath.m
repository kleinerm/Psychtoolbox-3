...
% We also add Psychtoolbox/PsychBasic/PsychPlugins to the dylib search path.
    ...
    if ~IsOSX(1) || ~IsOctave || ~compare_versions(version, '9.3.0', '==')
        error('osxsetoctaverpath only works with a 64-Bit version of HomeBrew Octave-9.3.0 for macOS!');
    end
    ...
    if IsARM
        libdir = '/opt/homebrew/opt/octave/lib/octave/9.3.0';
    else
        libdir = '/usr/local/opt/octave/lib/octave/9.3.0';
    end
