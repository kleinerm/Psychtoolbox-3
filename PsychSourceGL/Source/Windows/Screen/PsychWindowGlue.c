...
static void OptimizeDWMParameters(PsychWindowRecordType *windowRecord)
{
    ...
    if ((S_OK == DwmSetPresentParameters(windowRecord->targetSpecific.windowHandle, &dwmPresentParams)) &&
...
psych_bool PsychOSOpenOnscreenWindow(PsychScreenSettingsType *screenSettings, PsychWindowRecordType *windowRecord, int numBuffers, int stereomode, int conserveVRAM)
{
    ...
    // DWM enable/disable dance: No point in doing the query/disable/enable dance on
    // Windows-8 or later, as we don't have any control about it anymore:
    if (!PsychOSIsMSWin8()) {
        // This is Windows Vista or Windows-7. Do the dance.

        // Check current enable state:
        if (DwmIsCompositionEnabled(&compositorEnabled)) {
        ...
        if (DwmEnableComposition((compositorEnabled) ? 1 : 0)) {
            if (PsychPrefStateGet_Verbosity() > 1) {
        ...
        // Retest state:
        if (DwmIsCompositionEnabled(&compositorPostEnabled)) {
        ...
        if (compositorPostEnabled && (DwmEnableMMCSS(compositorPostEnabled))) {
            if (PsychPrefStateGet_Verbosity() > 1) {
    ...
    if (PsychOSIsMSWin8()) {
        ...
        if ((DwmEnableMMCSS(TRUE)) && (PsychPrefStateGet_Verbosity() > 1)) {
...
double PsychOSGetVBLTimeAndCount(PsychWindowRecordType *windowRecord, psych_uint64* vblCount)
{
    ...
    // now DwmGetCompositionTimingInfo returns info even in fullscreen
    ...
    #if 0
    if (PsychOSIsDWMEnabled(0) && (NULL != DwmGetCompositionTimingInfo) &&
        ((rc = DwmGetCompositionTimingInfo(NULL, &dwmtiming)) == 0)) {
        ...
        *vblCount = (psych_uint64) dwmtiming.cDXRefresh;
        ...
        if (PsychOSIsDWMEnabled(0) && PsychPrefStateGet_Verbosity() > 6) {
            printf("PTB-DEBUG: Call to DwmGetCompositionTimingInfo(%i) failed with rc = %x, GetLastError() = %i\n", dwmtiming.cbSize, rc, GetLastError());
        }
    }
    #endif
...
psych_bool PsychOSGetPresentationTimingInfo(PsychWindowRecordType *windowRecord, psych_bool postSwap, unsigned int flags, psych_uint64* onsetVBLCount, double* onsetVBLTime, psych_uint64* frameId, double* compositionRate, int fullStateStructReturnArgPos)
{
    ...
    // Windows Vista DWM available, supported and enabled?
    dwmtiming.cbSize = sizeof(dwmtiming);
    if ( PsychOSIsDWMEnabled(0) && ((rc1 = DwmGetCompositionTimingInfo(NULL, &dwmtiming)) == 0) ) {
        ...
        *compositionRate = (double) dwmtiming.rateCompose.uiNumerator / (double) dwmtiming.rateCompose.uiDenominator;
        ...
        if (PsychPrefStateGet_Verbosity() > 15) {
            printf("PTB-DEBUG: === DwmGetCompositionTimingInfo returned data follows: ===\n\n");
            ...
            printf("PTB-DEBUG: === End of DwmGetCompositionTimingInfo returned data.  ===\n\n");
        }
        ...
        if (PsychOSIsDWMEnabled(0) && PsychPrefStateGet_Verbosity() > 6) {
            printf("PTB-DEBUG: Call to DwmGetCompositionTimingInfo() failed with rc1 = %x, GetLastError() = %i\n", rc1, GetLastError());
        }
    }
...
int PsychOSIsDWMEnabled(int screenNumber)
{
    ...
    // check if it is really active, ie., compositing instead of just on standby. The DwmIsCompositionEnabled()
    // reports nominal enable state, but an enabled compositor still (should) get out of our way and therefore be
    // effectively disabled if a fullscreen window is displayed. A query to DwmGetCompositionTimingInfo() will
    // succeed if the DWM is really active, but fail if the DWM is on standby.
    IsDWMEnabled = ((0 == DwmIsCompositionEnabled(&compositorEnabled)) && (compositorEnabled || PsychOSIsMSWin8()) &&
                    (0 == DwmGetCompositionTimingInfo(NULL, &dwmtiming)));
    return(IsDWMEnabled);
}
...
psych_bool PsychOSSetPresentParameters(PsychWindowRecordType *windowRecord, psych_uint64 targetVBL, unsigned int queueLength, double rateDuration)
{
    ...
    // Call function if DWM is supported and enabled:
    if (PsychOSIsDWMEnabled(0) && ((rc = DwmSetPresentParameters(windowRecord->targetSpecific.windowHandle, &dwmPresentParams)) == 0)) return(TRUE);
...
psych_int64 PsychOSGetSwapCompletionTimestamp(PsychWindowRecordType *windowRecord, psych_int64 targetSBC, double* tSwap)
{
    ...
    while (TRUE) {
        ...
        if ((rc = DwmGetCompositionTimingInfo(NULL, &dwmtiming)) != 0) {
            if (PsychPrefStateGet_Verbosity() > 6) printf("PTB-DEBUG: PsychOSGetSwapCompletionTimestamp failed: Call to DwmGetCompositionTimingInfo() failed with rc = %x, GetLastError() = %i\n", rc, GetLastError());
            return(-1);
        }
