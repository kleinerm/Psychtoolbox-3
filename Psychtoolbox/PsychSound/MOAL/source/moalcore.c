...
/* Build and include support for alm if BUILD_ALM is defined.
   Otherwise, only build OpenAL wrappers.
*/
...
DEFUN_DLD(moalcore, inprhs, nlhs,
"moalcore - Octave core OCT file for 'OpenAL for Octave'.\n\n"
"You should *not* directly call moalcore, it is only called by\n"
"the OpenAL alXXX.m M-File wrappers in MOAL/wrap and by the InitializeMatlabOpenAL\n"
"M-File. The only interesting call you could make is moalcore('DEBUGLEVEL', x);\n"
"to specify a new level x of verbosity for debug output. If you want to call\n"
"any other moalcore subfunction, then make sure that you read and understand\n"
"moalcore's C source code!\n\n\n")
#endif
{
    // Start of dispatcher:
    int i;
...
#if PSYCH_LANGUAGE == PSYCH_OCTAVE
    #define const
    const char mexFunctionName[] = "moalcore";
    ...
    // we return immediately. moalcore('PREINIT') is called by ptbmoalinit.m
    ...
    // debuglevel > 0 --> Output glError()'s in clear-text and abort. Output ALSL errors and abort.
    ...
    #ifdef BUILD_GLM
    // ALM module is included and supported in moalcore: This is necessary if
    // one wants to use MOAL independent from Psychtoolbox. ALM is only supported
    // on MacOS-X, not on Linux or Windows...
    
    // We execute alm-commands without performing ALEW first-time initialization,
    // because to execute glewinit() we need a valid OpenAL context. This context is
    ...
    if (firsttime) {
        // Yes. Ready to go...
		if (debuglevel > 1) {
			printf("MOAL - OpenAL for Matlab & GNU/Octave initialized - MOAL is (c) 2007 Mario Kleiner & Richard F. Murray, licensed to you under GPL.\n");
		}
        fflush(NULL);
    ...
    if (glBeginLevel == 0 && debuglevel > 0 && strcmp(cmd, "alGetError")!=0) alGetError();
...
// As we use the ALEW library to dynamically detect and bind all OpenAL functions, we can
...
void* PsychEnqueueTempMemory(void* p, unsigned long n, int mlist)
{
  ...
  *((unsigned long*) p) = n;
  ...
  if (debuglevel > 1) mexPrintf("MOAL: Memlist %i : Allocated new buffer %p of %i Bytes,  new total = %i.\n", mlist, p, n, totalTempMemAllocated[mlist]); fflush(NULL);
...
void *PsychCallocTemp(unsigned long n, unsigned long size, int mlist)
{
  ...
  if(NULL==(ret=calloc((size_t) 1, (size_t) realsize))) {
    mexErrMsgTxt("MOAL-FATAL ERROR: Out of memory in PsychCallocTemp!\n");
  }
...
void *PsychMallocTemp(unsigned long n, int mlist)
{
  ...
  if(NULL==(ret=malloc((size_t) n))){
    mexErrMsgTxt("MOAL-FATAL ERROR: Out of memory in PsychMallocTemp!\n");
  }
...
void PsychFreeTemp(void* ptr, int mlist)
{
  ...
  if (PsychTempMemHead[mlist] == ptr) {
    ...
    if (debuglevel > 1) mexPrintf("MOAL: Memlist %i : Freed buffer at %p, new total = %i.\n", mlist, ptrbackup, totalTempMemAllocated[mlist]); fflush(NULL);
  ...
  printf("MOAL-BUG: In PsychFreeTemp: Tried to free non-existent temporary membuffer %p!!! Ignored.\n", ptrbackup);
  fflush(NULL);
  return;
}
...
void PsychFreeAllTempMemory(int mlist)
{
  ...
  if (totalTempMemAllocated[mlist] != 0) {
    printf("MOAL-CRITICAL BUG: Inconsistency detected in temporary memory allocator!\n");
    printf("MOAL-CRITICAL BUG: totalTempMemAllocated[%i] = %i after PsychFreeAllTempMemory(%i)!!!!\n",
  ...
  if (debuglevel > 1) printf("MOAL: Memlist %i : Freed all internal memory buffers.\n", mlist); fflush(NULL);
