...
if strcmpi(cmd, 'PerformPostWindowOpenSetup')
    ...
    % Mesa open-source Vulkan drivers of Mesa version < 30.0.0 in use for fullscreen direct display mode and
    ...
    if isFullscreen && any(windowRect(1:2)) && ismember(devs(gpuIndex).DriverId, [3, 6, 13, 18:20, 22:23]) && ...
       (devs(gpuIndex).DriverVersionRaw < bitshift (30, 22) + bitshift (0, 12) + 0)
        ...
        % present -> Boom!
        %
        % The bug has been diagnosed by myself and a bug fix by myself will be on the way to upstream shortly,
        % but likely won't make it before Mesa 22.2.2 the very earliest, possibly later, so we need a workaround
        % for at least Debian 11, Ubuntu 20.04-LTS and Ubuntu 22.04.1-LTS, possibly 22.04.2-LTS.
        %
        ...
        %fprintf('PsychVulkan-WARNING: Please upgrade to Mesa version 22.2.2 or later to get rid of this hack.\n');
    else
