function EyeLink_BufferedEndSacEvents(screenNumber)
...
%
% Usage:
% Eyelink_BufferedEndSacEvents(screenNumber)
%
...
try
    ...
    if ver ~=0 % If connected to EyeLink
    ...
    for i = 1:length(imgList)
        ...
        rt = 0;% Default value for first saccade reaction time
        ...
        while 1 % loop until error, space bar or button press            
            ...
            % This will clear old data from the buffer and allow access to the most recent online saccade event.
            if GetSecs - bufferStart > 0.1 && Sstim == 0 % If 100ms have elapsed and initial trial image has not yet been presented...                
                ...
                % Return the current EDF time (in seconds) to make sure we use only online saccades that started after stimulus presentation
                StimEDFtime = (Eyelink('TrackerTime'))*1000; % Multiply by 1000 to convert to milliseconds                               
                Sstim = 1; % Initial stimulus has been drawn
            end
            
            % STEP 5.5: DRAW GAZE-CONTINGENT DOT; WAIT FOR KEYPRESS/BUTTON; SHOW BLANK SCREEN; STOP RECORDING
            ...
            % Read item type returned by getnextdatatype. Wait for end of saccade (ENDSACC) event
            ...
            if evtype == el.ENDSACC % if end of saccade (ENDSACC) event is returned
                ...
                if Sstim == 1 % Only draw gaze-contingent dot after initial trial image is presented (i.e. after looping for ~100ms)                   
                    ...
                    % Use event only if it is from the expected eye and start of saccade (evt.sttime) occurred after trial image onset
                    if evt.eye == eyeUsed && evt.sttime > StimEDFtime                        
                        eventCount = eventCount + 1; % Add 1 to saccade event counter                        
                        % Write message to EDF file to mark time when end of saccade is detected (before drawing of gaze-contingent target)
                        Eyelink('Message', 'END_SACCADE_DETECTED_%d', eventCount);                                                
                        % Save event fields as variables. See EyeLink Programmers Guide manual > Data Structures > FEVENT
                        sac_start = evt.sttime; % Saccade start EDF time
                        x_end = evt.genx; % Saccade end x gaze position
                        y_end = evt.geny; % Saccade end y gaze position
                        
                        % The following event properties are not used in this demo but are available from the ENDSACC event:
                        % evt.type; % Event type (STARTBLINK=3, ENDBLINK=4, STARTSACC=5, ENDSACC=6, STARTFIX=7, ENDFIX=8, FIXUPDATE=9, SAMPLE =200)
                        % evt.eye; % Event eye: 0=left eye, 1=right eye
                        % evt.entime; % Saccade end EDF time
                        % evt.gstx; % Saccade start x gaze position
                        % evt.gsty; % Saccade start y gaze position
                        % evt.supd_x; % Saccade start x 'pixel per deggree' value
                        % evt.supd_y; % Saccade start y 'pixel per degree' value
                        % evt.eupd_x; % Saccade end x 'pixel per degree' value
                        % evt.eupd_y; % Saccade end y 'pixel per degree' value
                        
                        % Draw gaze-contingent dot
                        Screen('DrawTexture', window, stimTexture); % Prepare stimulus texture on backbuffer
                        % Superimpose a red gaze-contingent dot using saccade end position coordinates
                        Screen('FillOval', window, [255,0,0], [x_end-20, y_end-20, x_end+20, y_end+20]);
                        Screen('Flip', window); % Present stimulus
                        % Write message to EDF file to mark time of new target drawing.
                        Eyelink('Message', 'TARGET_NEW_POS_%d', eventCount);
                        ...
                        Eyelink('Message', '!V FIXPOINT %d %d %d %d %d %d %d %d %d %d', 255, 0, 0, 255, 0, 0, round(x_end), round(y_end), 40, 40, x_end, y_end);
                        % Calculate start of first saccade reaction time
                        if eventCount == 1
                            rt = round(sac_start - StimEDFtime);
                        end                        
                    end
                end
            end            
        ...
        Eyelink('Message', '!V TRIAL_VAR event_count %d', eventCount);% Number of online ENDSACC events per trial
        WaitSecs(0.001); % Allow some time between messages. Some messages can be lost if too many are written at the same time
        Eyelink('Message', '!V TRIAL_VAR rt_firstSac %d', rt);% Reaction time: first saccade start time relative to stim onset        
