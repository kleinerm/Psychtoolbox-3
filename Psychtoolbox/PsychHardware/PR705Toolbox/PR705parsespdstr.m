...
if ~isempty(rawspd)
    ...
    if length(wls) ~= 201 % 201 is the PR-705 default
        error('Unexpected native number of wavelength samples for PR-705!');
    elseif wls(2) - wls(1) ~= 2 || ~all(diff(wls) == 2)
        error('Unexpected native wavelength sampling for the PR-705!');
    end
